...
const phrases = ["Welcome to Malyx Shop"];
let phraseIndex = 0;
let charIndex = 0;
...
function type() {
    ...
    if (!isDeleting && charIndex === currentPhrase.length) {
        typeSpeed = 2000; // සම්පූර්ණ වචනය ටයිප් වූ පසු මදක් නතර වේ
        isDeleting = true;
...
function searchProducts() {
    let input = document.getElementById('searchInput').value.toLowerCase();
    let products = document.getElementsByClassName('product-name'); // ඔබේ නිෂ්පාදනයේ නම ඇති class එක

    for (let i = 0; i < products.length; i++) {
        let productName = products[i].textContent.toLowerCase();
        
        // අකුරු දෙක මුලටම තියෙන ඒවා පමණක් පෙන්වීමට:
        if (productName.startsWith(input)) {
            products[i].parentElement.style.display = ""; // පෙන්වන්න
        } else {
            products[i].parentElement.style.display = "none"; // සඟවන්න
        }
    }
}